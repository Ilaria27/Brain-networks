# -*- coding: utf-8 -*-
"""
Created on Tue Apr 20 14:05:55 2021
...
#subjects_indx    = pd.read_csv('Data Description/selected_subjects_27.csv', header=None)
...
def parse_args():
    ...
    parser.add_argument('-c1', dest = 'c1', help = 'class 1 of dataset')
    parser.add_argument('-c2', dest = 'c2', help = 'class 2 of dataset')
...
if connectivity=="Structural":
    ...
    features = []
    for g in groups:
        testfiledir = '../Dataset/' + dataset + '/' + g
        files = os.listdir(testfiledir)
        nfiles = len(files)
        
        asd = []
        for filename in files:
            m = pd.io.parsers.read_csv(testfiledir + '/' + filename, header = None)
            asd.append(m.values)
        
        asd_t = []
        for matrix in asd:
            asd_t.append(np.triu(matrix, k = 1))
            
        m = []
        for n in range(len(asd_t)):
            A = asd_t[n]
            m.append(A[np.triu_indices(len(A), k = 1)])
        m = np.array(m)
        features.append(m)
...
for i in range(49, labels.shape[0]):
    labels[i] = 0
"""
Parameters
"""
...
scipy.io.savemat(SAVING_MAT+'GlobalResults_'+resolution+'_'+connectivity+'_'+scoring+'_'+dataset+'.mat', data1); print("Saving .mat...");
